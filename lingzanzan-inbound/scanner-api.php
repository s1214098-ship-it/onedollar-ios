...
function scanner_rows(array $products,array $skus,string $query,array $sessions,array $barcodeAliases=[]): array {
    ...
    foreach($skus as $sku){ if(!is_array($sku))continue; $product=$productMap[(string)($sku['productId']??'')]??[];
        ...
        // LZ_XFER_SELECT_20260926: product-code / series hits (PAN001) must stay
        // selectable size cards. Putting product.code in lookup codes used to
        // mark every size exact, then the identity-conflict lock disabled them.
        foreach($codes as $code){
            if(!scanner_query_matches_code($q,$code)) continue;
            $codeNorm=scanner_norm($code);
            $codeProduct=scanner_quick_product_code($code);
            if($seriesMatch && $codeProduct===$q && $codeNorm===$q) continue;
            $exact=true;
        }
        if($q!==''&&!$seriesMatch&&!$exact&&strpos($hay,$q)===false)continue;
...
function scanner_unique_exact_sku_index(array $products,array $skus,string $query,array $sessions,array $barcodeAliases,string $warehouse=''): ?int {
    ...
    $productIds=[];
    foreach ($exactRows as $row) {
        $pid=scanner_norm($row['productId'] ?? '');
        if($pid!=='') $productIds[$pid]=true;
    }
    // Same product, multiple sizes/colors is a picker — not a barcode conflict.
    if (count($productIds) > 1) {
        scanner_response([
            'ok'=>false,
            'error'=>'此掃描碼同時對到不同產品，已停止寫入庫存。請改掃 QR 完整條碼，或由管理者先修正條碼歸屬。',
...
function scanner_rows_have_identity_conflict(array $rows): bool {
    // LZ_XFER_SELECT_20260926: lock only when one barcode maps to different
    // products. One product with XL/S/M/L/2XL must stay tappable in 調撥.
    $productIds=[];
    foreach($rows as $row){
        if(!is_array($row)||empty($row['exact']))continue;
        $pid=scanner_norm($row['productId']??'');
        if($pid==='') $pid=scanner_norm($row['productCode']??'');
        if($pid!=='') $productIds[$pid]=true;
    }
