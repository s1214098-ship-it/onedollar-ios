(function () {
  ...
  function standaloneLinePhotoHtml(line) {
    ...
    return [
      ...
      '<div class="purchase-receipt-photo-head"><b>到貨照片</b></div>',
      ...
      '<button type="button" class="ghost-button photo-action-camera" data-standalone-photo-camera>拍照</button>',
      '<button type="button" class="ghost-button photo-action-pick" data-standalone-photo-pick>選擇檔案</button>',
      '<button type="button" class="ghost-button photo-action-paste" data-standalone-line-paste>貼上圖片</button>',
      '<button type="button" class="ghost-button photo-action-rotate" data-standalone-photo-rotate="-90">左轉</button>',
      '<button type="button" class="ghost-button photo-action-rotate" data-standalone-photo-rotate="90">右轉</button>',
      '<button type="button" class="ghost-button photo-action-clear" data-standalone-photo-clear>清除</button>',
      '</div>',
  ...
  var QUICK_INBOUND_HINT_ON = '';
  var QUICK_INBOUND_HINT_OFF = '';
  ...
  function renderStandaloneLines() {
    ...
    host.innerHTML = standaloneColorDatalistHtml() + state.standaloneLines.map(function (line, index) {
      ...
      return [
        ...
        '<label class="purchase-receipt-line-name">產品名稱<input value="' + escapeHtml(line.productName || line.productCode || '') + '" data-standalone-line-product-name></label>',
        ...
        '<label class="purchase-receipt-line-barcode">公司條碼<input value="' + escapeHtml(line.barcode || '') + '" data-standalone-line-barcode data-barcode-auto="' + (line.barcodeAuto ? '1' : '0') + '" data-barcode-manual="' + (line.barcodeManualEdited ? '1' : '0') + '"></label>',
  ...
  function setReceivedPrintPage(on, summary) {
    ...
    if (complete) {
      ...
      if (metaEl) metaEl.textContent = summary.historical
        ? ([warehouseLabel + ' 已入庫 ' + receivedQty + ' 件', text(summary.supplier), text(summary.sourceOrder)].filter(Boolean).join(' · '))
        : (warehouseLabel + ' 已增加 ' + receivedQty + ' 件庫存');
