...
function stockinCode128Svg(value) {
  ...
  function queueTransfer(row, code) {
    ...
    if (row && !sameWarehouse(row.warehouse, from) && stockOf(row) > 0 && transferMode() !== 'computer') {
      ...
      if (sourceWarehouse) sourceWarehouse.value = from;
      if (from && to && from === to) {
        to = oldFrom;
        if (targetWarehouse) targetWarehouse.value = to;
      }
      switchedNote = '來源倉已改為「' + from + '」→「' + to + '」。';
    }
    ...
    if (existingIndex !== -1) {
      ...
      saveTransferDraft();
      beep(1);
      setMessage('一聲：' + switchedNote + '已直接加入本次調撥單 ' + scanQty + ' 件；掃完請按「儲存整張調撥單」。', 'ok');
    }
    resetProductSelection();
    readyForNextScan();
  }
  ...
  // LZ_XFER_SELECT_20260926: same-product size cards are a picker, not a lock.
  function lookupHasProductConflict(rows) {
    var ids = {};
    (rows || []).forEach(function (row) {
      if (!row || !row.exact) return;
      var id = String(row.productId || row.productCode || '').trim();
      if (id) ids[id] = true;
    });
  ...
  function renderMatches(rows) {
    ...
    matchGrid.innerHTML = visible.map(function (row, index) {
      ...
      return '<button type="button" class="match-card' + (selected === row ? ' is-selected' : '') + '" data-match-index="' + index + '"><span class="match-card-photo"><img src="' + escapeHtml(resolveImage(row.image)) + '" alt="商品對照圖" draggable="false"></span><span><b>' + escapeHtml(row.productCode) + '</b><em>' + escapeHtml(row.title) + '</em><strong>' + escapeHtml(row.color) + ' · ' + escapeHtml(row.size) + '</strong><small>分類：' + escapeHtml(row.category || '未設定分類') + '</small><small>' + escapeHtml(row.warehouse) + '／庫存 ' + stockOf(row) + ' 件</small><small>倉位：' + escapeHtml([row.shelf || '未設定貨架',row.layer || '未設定層位'].join('／')) + '</small><small>' + escapeHtml(stockPurposeText(row)) + '</small><small>' + escapeHtml(barcodeCheck) + '</small><small>' + escapeHtml(rowCostText(row)) + '</small><small>' + escapeHtml(last) + '</small><small class="target-stock">本次 ' + escapeHtml(wh === 'all' ? '全部倉庫' : wh) + ' 盤點前：' + targetStock + ' 件</small><code>' + escapeHtml(row.skuId) + '</code></span></button>';
    }).join('');
  }
  ...
  function classifyLookup(rows, code) {
    ...
    if (currentTab !== 'stockin' && !warehouseRows.length) {
      if (currentTab === 'transfer' && otherWarehouseStockRows.length) {
        warehouseRows = prioritizedRows(rows);
      } else {
    ...
    if (currentTab === 'transfer' && warehouseRows.every(function (row) { return stockOf(row) <= 0; })) {
      if (otherWarehouseStockRows.length) {
        warehouseRows = prioritizedRows(rows);
      } else {
  ...
  function lookup(code, options) {
    ...
    autoStagePendingBeforeNextScan(code).then(function () {
      ...
      .then(function (payload) {
        var rows = payload && Array.isArray(payload.rows) ? payload.rows : [];
        if (payload && payload.ambiguousBarcode && lookupHasProductConflict(rows)) {
          queuedLookupCounts[code] = 0;
          resetProductSelection();
          renderMatches(rows);
          document.querySelectorAll('[data-match-index]').forEach(function (card) { card.disabled = true; });
          if (matchPanel) matchPanel.hidden = false;
          archiveScanResult(code, 'ambiguous_blocked', null, rows.length, '掃描碼同時屬於不同產品，已禁止選用');
          beep(3);
  ...
  document.addEventListener('click', function (event) {
    ...
    var matchCard = event.target.closest && event.target.closest('[data-match-index]');
    var target = matchCard || (event.target.closest && event.target.closest('button'));
    if (!target) return;
