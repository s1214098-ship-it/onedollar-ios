<?php
if (($_GET['k'] ?? '') !== 'recv-keep-cat-swap-20260926-1') {
...
try {
  echo 'js=' . copy_swap($assets . '/admin.js.new-recv-cat-1', $assets . '/admin.js') . "\n";
  echo 'freightHtml=' . copy_swap($root . '/admin-freight.html.new-recv-cat-1', $root . '/admin-freight.html') . "\n";
  echo 'ordersHtml=' . copy_swap($root . '/admin-orders.html.new-recv-cat-1', $root . '/admin-orders.html') . "\n";
  ...
  echo 'jsNoForceRepick=' . (strpos($js, '跨部門時必須重新選擇該部門的分類') === false ? 'yes' : 'no') . "\n";
  echo 'freightBust=' . (strpos($freight, '20260926-recv-cat-1') !== false ? 'yes' : 'no') . "\n";
  echo 'ordersBust=' . (strpos($orders, '20260926-recv-cat-1') !== false ? 'yes' : 'no') . "\n";
  echo "ok\n";
} catch (Exception $e) {
