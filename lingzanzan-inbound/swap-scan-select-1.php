<?php
if (($_GET['k'] ?? '') !== 'scan-select-swap-20260926-1') {
...
try {
  echo 'js=' . copy_swap($assets . '/inventory-freight-entry-20260810.js.new-scan-select-1', $assets . '/inventory-freight-entry-20260810.js') . "\n";
  echo 'html=' . copy_swap($root . '/admin-inventory-entry.html.new-scan-select-1', $root . '/admin-inventory-entry.html') . "\n";
  $js = file_get_contents($assets . '/inventory-freight-entry-20260810.js') ?: '';
  $html = file_get_contents($root . '/admin-inventory-entry.html') ?: '';
  echo 'jsMarker=' . (strpos($js, 'LZ_SCAN_ANYWHERE_20260926') !== false ? 'yes' : 'no') . "\n";
  echo 'jsColorMain=' . (strpos($js, 'LZ_COLOR_AS_MAIN_20260926') !== false ? 'yes' : 'no') . "\n";
  echo 'htmlBust=' . (strpos($html, '20260926-scan-select-1') !== false ? 'yes' : 'no') . "\n";
  echo "ok\n";
} catch (Exception $e) {
