...
/**
 * 出貨核對單把寄庫從⑤搬到②客戶資料下面，並加「加入寄庫名單」按鈕。
 * 已在寄庫的單顯示「此單已在寄庫名單」，不必再往下翻找。
 *
...
const STAMP = "20260822-fifo-hold-1";
...
const MOVE_FROM_NEW = `<h4 class="freight-fifo-shipping-title">⑤ 正式出貨資料</h4><p class="freight-fifo-hold-jump">寄庫與預約出貨在上面「② 客戶資料」下面，選原因後按「加入寄庫名單」。這裡只填物流公司與單號。</p><div class="freight-fifo-shipping-fields">`;
...
if (admin.indexOf(JS_MARKER) !== -1 && admin.indexOf("此單已在寄庫名單") !== -1 && admin.indexOf("function saveFreightFifoHold") !== -1) {
  ...
  admin = replaceOnce(admin, INSERT_BEFORE_OLD, INSERT_BEFORE_NEW, "insert reservation under customer");
...
if (admin.indexOf('class="freight-fifo-reservation"') === -1) throw new Error("reservation block missing");
